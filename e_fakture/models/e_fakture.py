...
class EFakture(models.Model):
    ...
    def import_vendor_bills(self):
        ...
        account_id = params.get_param('e_fakture.e_account_id')
        ...
        if response.status_code == 200:
            ...
            for invoice_id in purchase_invoice_ids:
                ...
                if invoice_details.get('env:DocumentEnvelope').get('env:DocumentBody').get('Invoice'):
                    print("##############################################################################################")
                    pprint(invoice_details.get('env:DocumentEnvelope').get('env:DocumentBody').get('Invoice').get('cac:InvoiceLine'), depth=4)
                    print("##############################################################################################")
                    ...
                    if partner_details:
                        ...
                        try:
                            ...
                            party_name = partner_details.get('cac:PartyLegalEntity').get('cbc:RegistrationName')
                        ...
                        try:
                            ...
                            country_code = adress.get('cbc:CountrySubentity')
                        ...
                        due_date = bill.get('cbc:DueDate')
                        if not due_date:
                            due_date = bill.get('cbc:IssueDate')
                        ...
                        partner_id = self.env['res.partner'].search([('id', '=', partner)], limit=1)
                        if partner_id.account_id_e.id:
                            account_id = partner_id.account_id_e.id
                        if type(lines) != list:
                            lines = [lines]
                        ...
                        for line in lines:
                            ...
                            account_lines = {
                                ...
                                'price_unit': line.get('cac:Price').get('cbc:PriceAmount').get('#text'),
                                'tax_ids': vat,
